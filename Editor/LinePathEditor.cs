...
using LinePathCreation;
using LinePathCreation.Utility;
using UnityEditor;
using UnityEditor.IMGUI.Controls;
using UnityEngine;

namespace LinePathCreationEditor
{
    ...
    public class LinePathEditor : Editor
    {
        ...
        static readonly string[] spaceNames = { "3D (xyz)", "2D (xy)", "Top-down (xz)", "Line (xy)" };
                ...
                if (data.showPathOptions)
                {
                    ...
                    if (data.showTransformTool & (handleIndexToDisplayAsTransform == -1))
                    {
                        if (GUILayout.Button("Centre Transform"))
                        {
                            ...
                            if (bezierPath.Space == PathSpace.xy || bezierPath.Space == PathSpace.line)
                            {
        ...
        void DrawVertexPathSceneEditor()
        {
            ...
            if (data.showBezierPathInVertexMode)
            {
                for (int i = 0; i < bezierPath.NumSegments; i++)
                {
                    ...
                    if (bezierPath.Space == PathSpace.line)
                    {
        ...
        void DrawBezierPathSceneEditor()
        {
            ...
            bool displayControlPoints = data.displayControlPoints && (bezierPath.ControlPointMode != BezierPath.ControlMode.Automatic || !globalDisplaySettings.hideAutoControls) && bezierPath.Space != PathSpace.line;
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                // Draw normals
                if (data.showNormals && bezierPath.Space != PathSpace.line)
                {
            ...
            // Disable handle in line pathSpace
            if (data.displayControlPoints && bezierPath.Space != PathSpace.line)
            {
        ...
        void OnEnable()
        {
            creator = target as LinePathCreator;
            if (creator == null)
            {
                return;
            }
